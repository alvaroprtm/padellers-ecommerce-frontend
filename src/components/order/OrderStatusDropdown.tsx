...
export const OrderStatusDropdown = ({
  ...
  const getAvailableStatuses = () => {
    switch (currentStatus) {
      ...
      case 'paid':
        return statusOptions.filter(option =>
          ['shipped', 'cancelled'].includes(option.value)
        );
      case 'shipped':
        return statusOptions.filter(option =>
          ['completed'].includes(option.value)
        );
      case 'completed':
        return [];
      case 'cancelled':
  ...
  if (availableStatuses.length === 0 || disabled) {
    return (
      <div
        className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border bg-gray-700 border-gray-600 ${currentOption.color}`}
      >
        {currentOption.label}
